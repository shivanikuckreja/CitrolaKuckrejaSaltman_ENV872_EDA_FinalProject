...
mapView(counties_sf, zcol = "AWATER") #zcol - colors map by desired attribute
```
...
upper_neuse <- st_read("./Data/Spatial/NCHUC8.shp") %>%
  filter(SUBBASIN == "Upper Neuse")

#Create a map
ggplot() +
  geom_sf(data=huc8_sf, fill = 'orange') +
  geom_sf(data = upper_neuse, fill = 'purple')


mapview(huc8_sf, col.regions = 'orange') +
  mapview(upper_neuse, col.regions = 'purple')
```
...
mapview(counties_sf_join, 
  ...
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #cex shows size based on attribute 
...
huc_122 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #just take certain amount of a string of digits
  group_by(HUC_8) %>%
  summarise(kWhkm2day = sum(kWhkm2day))

#Join to HUC_8 features

hub812join <- huc8_sf %>%
  left_join(huc_122, by = c("HUC_8" = "HUC_8"))

#View the outputs
mapview(hub812join, zcol = "kWhkm2day")
...
#Map the data
mapview(huc2_sf, zcol = "DWQ_Basin")
```

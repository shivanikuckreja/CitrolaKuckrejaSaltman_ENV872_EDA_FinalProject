---
title: 'Sam Saltman: 11: Crafting Reports'
...
1. _**Easy format** for readers_  
2. _Ability to **generate a report**_  
3. _**Reproducibility**_  
4.  _Ability to **collaborate** connecting Rmd to github_  
5. _Can use **multiple languages**_
...
------- | ---------
EPA Air | Air qualtiy
NTL-LTER Lake | Lake Data
Wind Speed | Port Arthur Texas Wind
...
```{r echo=FALSE}
NTL_lake_nutrient <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv", stringsAsFactors = TRUE) 
head(NTL_lake_nutrient)
NTL_lake_nutrient$sampledate <- as.Date(NTL_lake_nutrient$sampledate, format = "%m/%d/%y")
```
...
```{r}
NTL_lake_nutrient_processed <- NTL_lake_nutrient %>%
...
Nitrogen <- NTL_lake_nutrient_processed %>%
  group_by(lakename) %>%
  summarise(Mean = mean(tn_ug), Minimum = min(tn_ug), Maxiumum = max(tn_ug), StandardDeviation = sd(tn_ug))


Phosphorus <- NTL_lake_nutrient_processed %>%
  group_by(lakename) %>%
  summarise(Mean = mean(tp_ug), Minimum = min(tp_ug), Maxiumum = max(tp_ug), StandardDeviation = sd(tp_ug))
```
...
```{r echo=FALSE}
kable(Nitrogen, caption = 'Nitrogen Summary')

kable(Phosphorus, caption = 'Phosphorus Summary')
```
...
\newpage
```{r Nitrogen, echo=FALSE, fig.align = 'left', fig.cap = "Nitrogen Summary"}
library(RColorBrewer)

NitrogenPlot <- ggplot(NTL_lake_nutrient_processed) +
...
\newpage
```{r echo=FALSE, fig.align = 'left', fig.cap = "Phosphorus Summary"}
PhosphorusPlot <- ggplot(NTL_lake_nutrient_processed) +
...
The results indicate that nitrogen and phosphorous are present in all the lakes. Some of the lakes saw significant spikes between the years 1996-1998. There is the possibility that there was an extreme event that increased runoff high in both phosphorous and nitrogen during this period.

******


The next step is to analyze periods of spikes. We may want to look into seasonal data, and we may want to include additional datasets on upstream anthropogenic activities. The goal of the next steps is to determine how to prevent a significant spike in nitrogen and phosphorous in these lakes. 

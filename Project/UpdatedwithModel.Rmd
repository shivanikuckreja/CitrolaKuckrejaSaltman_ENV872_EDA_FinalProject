...
```{r echo=FALSE, tab.cap = "Data Summary"}
kable(sapply(Processed1_Filter, class))
```
...
The ‘logit3’ and ‘logit4’ models evaluated which chemical types have a lethal effect on bee and non bee species. The results of ‘logit3’ showed that nearly all chemical types with data had a significant effect on bees. However, the only chemical that was not significant (150824478) only had one entry. The results of ‘logit4’ showed that no chemical had a significant effect on non-bee species. The results of the four models are summarized in Table 1. 
...
# Appendix
```{r echo=FALSE, fig.cap = "Residual Plot for Bee Mortality by Exposure Type"}
par(mfrow=c(2,2))
plot(logit)
```

```{r echo=FALSE, fig.cap = "Residual Plot for Non-Bee Mortality by Exposure Type"}
par(mfrow=c(2,2))
plot(logit2)
```

```{r echo=FALSE, fig.cap = "Residual Plot for Bee Mortality by Chemical Type"}
par(mfrow=c(2,2))
plot(logit3)
```

```{r echo=FALSE, fig.cap = "Residual Plot for Non-Bee Mortality by Chemical Type"}
par(mfrow=c(2,2))
plot(logit4)
```
